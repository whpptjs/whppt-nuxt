...
export default (context, inject) => {
  const whppt = (global.$whppt = {
    ...
    addPlugin(plugin) {
      ...
      if (plugin.pageType && !plugin.pageType.createPage)
        plugin.pageType.createPage = createPage(plugin.pageType.name, options);
      if (plugin.pageType && !plugin.pageType.deletePage) plugin.pageType.deletePage = deletePage;
      if (plugin.pageType && !plugin.pageType.loadPage)
        plugin.pageType.loadPage = loadPage(plugin.pageType.name, options);
