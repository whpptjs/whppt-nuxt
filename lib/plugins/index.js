...
export default (context, inject) => {
  const whppt = (global.$whppt = {
    ...
    addPlugin(plugin) {
      const collection = plugin.collection || plugin.pageType.name;

      if (plugin.pageType && !plugin.pageType.createPage) plugin.pageType.createPage = createPage(collection);
      if (plugin.pageType && !plugin.pageType.deletePage) plugin.pageType.deletePage = deletePage(collection);
      if (plugin.pageType && !plugin.pageType.loadPage) plugin.pageType.loadPage = loadPage(collection);
      if (plugin.pageType && !plugin.pageType.savePage) plugin.pageType.savePage = savePage(collection);
      if (plugin.pageType && !plugin.pageType.checkSlug) plugin.pageType.checkSlug = checkSlug(collection);
      if (plugin.pageType && !plugin.pageType.publishPage) plugin.pageType.publishPage = publishPage(collection);
      if (plugin.pageType && !plugin.pageType.unpublishPage) plugin.pageType.unpublishPage = unpublishPage(collection);
      /* No defaults being set for stripSlug and formatSlug currently */
  ...
  const dynamicDirectives = [
    ...
    { name: 'whppt-link', componentName: 'LinkEditor' },
