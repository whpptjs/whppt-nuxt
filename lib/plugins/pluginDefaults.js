...
function loadPage(collection) {
  return function({ $axios, $whppt }, { slug, domainId }) {
    return $axios.$get(`${$whppt.apiPrefix}/page/load?slug=${slug}&collection=${collection}&domainId=${domainId}`);
  };
}
...
function checkSlug(collection) {
  return function({ $axios, $whppt }, { slug, _id, domainId }) {
    return $axios.$get(
      `${$whppt.apiPrefix}/page/checkSlug?slug=${slug}&id=${_id}&collection=${collection}&domainId=${domainId}`
    );
  };
}
...
function publishPage(collection, beforePublish = () => Promise.resolve(), afterPublish = () => Promise.resolve()) {
  return function(context, { page }) {
    ...
    return beforePublish(context, { collection, id: page._id }).then(() => {
      return $axios.$post(`${$whppt.apiPrefix}/page/publishPage`, { page, collection }).then(publishedPage => {
        return afterPublish(context, { collection, id: page._id }).then(() => publishedPage);
      });
    });
  };
}
