...
function unpublishPage(collection, beforeUnpublish = Promise.resolve(), afterUnpublish = Promise.resolve()) {
  return function({ $axios, $whppt }, { _id }) {
    return beforeUnpublish().then(() => {
      return $axios.$post(`${$whppt.apiPrefix}/page/unpublishPage`, { _id, collection }).then(() => {
        return afterUnpublish();
      });
    });
  };
}
