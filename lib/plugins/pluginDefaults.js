...
function publishPage(collection) {
  return function({ $axios, $whppt }, { page }) {
    return $axios.post(`${$whppt.apiPrefix}/page/publishPage`, { page, collection });
  };
}

function unpublishPage(collection) {
  return function({ $axios, $whppt }, { _id }) {
    return $axios.$post(`${$whppt.apiPrefix}/page/unpublishPage`, { _id, collection });
  };
}
