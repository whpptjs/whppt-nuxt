...
export default options => ({
  ...
  actions: {
    ...
    publishPage({ commit, state }, { page } = {}) {
      ...
      return pageTypePlugin.pageType.publishPage(this.$whppt.context, { page: _page }).then(publishedPage => {
        commit('PAGE_PUBLISHED', {
          lastPublished: publishedPage.lastPublished || _page.lastPublished,
          published: publishedPage.published || _page.published,
        });
  ...
  mutations: {
    ...
    PAGE_PUBLISHED(state, changes) {
      state.page = { ...state.page, ...changes, published: true };
      state.savedPage = cloneDeep(state.page);
    },

    PAGE_SAVED(state, pageInfo) {
      state.page = { ...state.page, ...pageInfo };
      state.savedPage = cloneDeep(state.page);
    },
